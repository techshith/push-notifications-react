...
public class ProximityManager {
    ...
    private void initProximityWakeLock() {
        ...
        try {
            ...
            if (isSupported) {
                proximityWakeLock = powerManager.newWakeLock(proximityScreenOffWakeLock, TwilioVoiceModule.TAG);
    ...
    private void turnScreenOn() {
        if (proximityWakeLock == null) {
            if (BuildConfig.DEBUG) {
                Log.d(TwilioVoiceModule.TAG, ERROR_PROXIMITY_LOCK_NOT_SUPPORTED);
            }
            return;
        }
        synchronized (proximityWakeLock) {
            if (proximityWakeLock.isHeld()) {
                if (BuildConfig.DEBUG) {
                    Log.d(TwilioVoiceModule.TAG, "turnScreenOn()");
                }
                if (android.os.Build.VERSION.SDK_INT >= 21) {
    ...
    private void turnScreenOff() {
        ...
        synchronized (proximityWakeLock) {
            if (!proximityWakeLock.isHeld()) {
                if (BuildConfig.DEBUG) {
                    Log.d(TwilioVoiceModule.TAG, "turnScreenOff()");
                }
    ...
    private void initProximitySensorEventListener() {
        ...
        if (BuildConfig.DEBUG) {
            Log.d(TwilioVoiceModule.TAG, "initProximitySensorEventListener()");
        }
        proximityListener = new SensorEventListener() {
            ...
            public void onSensorChanged(SensorEvent sensorEvent) {
                if (sensorEvent.sensor.getType() == Sensor.TYPE_PROXIMITY) {
                    ...
                    eventManager.sendEvent(EventManager.EVENT_PROXIMITY, data);
                }
            }
    ...
    public void startProximitySensor() {
        ...
        if (proximityListener != null) {
            if (BuildConfig.DEBUG) {
                Log.d(TwilioVoiceModule.TAG, "register proximity listener");
            }
    ...
    public void stopProximitySensor() {
        ...
        if (proximityListener != null) {
            if (BuildConfig.DEBUG) {
                Log.d(TwilioVoiceModule.TAG, "unregister proximity listener");
            }
